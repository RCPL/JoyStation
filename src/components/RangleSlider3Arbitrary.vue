<template>
  <div class="slider">
    <svg width="300" class="three-way" id="threeWay" viewBox="0 0 300 300" v-on="{mousedown:down, touchstart:down, mousemove:move, touchmove:move, mouseup:up, touchend:up}">
      <polygon points="150 50, 250 220, 50 220"/>
      <circle :cx="x" :cy="y" r="15" v-if="activated" />
    </svg>
    <p>Option 1: <input type="range" v-model="option1" min="0" max="1" step="0.01"></p>
    <p>Option 2: <input type="range" v-model="option2" min="0" max="1" step="0.01"></p>
    <p>Option 3: <input type="range" v-model="option3" min="0" max="1" step="0.01"></p>
  ...
  export default {
    ...
    methods: {
      ...
      sameSide: function(p1,p2,a,b){
        let cp1 = this.cross(b.subtract(a.x,a.y,true), p1.subtract(a.x,a.y,true));
        let cp2 = this.cross(b.subtract(a.x,a.y,true), p2.subtract(a.x,a.y,true));
